...
class WatchStore:
    def __init__(self, path: str = "data/watchlist.sqlite3") -> None:
        ...
        with self._db:
            self._db.execute("""CREATE TABLE IF NOT EXISTS watchlist(
                ...
                last_digest TEXT, last_checked_at TEXT,
    ...
    def update_snapshot(self, chat_id: int | str, query: str, snapshot: str) -> bool:
        ...
        with self._lock, self._db:
            ...
            self._db.execute("UPDATE watchlist SET last_digest=?, last_checked_at=? WHERE chat_id=? AND query=?",
                             (digest, datetime.now(UTC).isoformat(), str(chat_id), query))
            return changed
